...
import {
  personalizeCardText,
} from '../utils/personalize-card-text';
  ...
  it('resuelve variables dentro del mensaje de ritmo', () => {
    expect(
      personalizeCardText({
        ...base,
        text:
          '{{actor}}, seguí provocando a {{target}}.',
      }),
  ...
  it('elige la forma femenina', () => {
    expect(
      personalizeCardText({
        ...base,
        text:
          'No pares hasta hacerla o hacerlo acabar.',
      }),
  ...
  it('elige la forma masculina', () => {
    expect(
      personalizeCardText({
        ...
        targetSex: 'hombre',
        text:
          'No pares hasta hacerlo o hacerla acabar.',
      }),
  ...
  it('resuelve pronombres declarados', () => {
    expect(
      personalizeCardText({
        ...base,
        text:
          'Acercate a {{target}} y no dejes de mirar{{target_object}}.',
      }),
    ).toBe(
      'Acercate a Ann y no dejes de mirarla.',
    );
  });
  ...
  it('no deja variables visibles', () => {
    expect(
      personalizeCardText({
        ...base,
        text:
          '{{actor}} avanzá. {{variable_desconocida}}',
      }),
    ).toBe('Ale avanzá.');
  });
