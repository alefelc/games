...
describe('contenido inicial', () => {
  it('contiene únicamente el catálogo público y referencias válidas', () => {
    ...
    expect(bundle.deckCards.every((row) => cardIds.has(row.card) && deckIds.has(row.deck))).toBe(true);
    expect(bundle.cardElements.every((row) => cardIds.has(row.card) && elementIds.has(row.element))).toBe(true);
    expect(bundle.cardToys.every((row) => cardIds.has(row.card) && toyIds.has(row.toy))).toBe(true);
    expect(bundle.cardTags.every((row) => cardIds.has(row.card) && tagIds.has(row.tag))).toBe(true);
  });
});
