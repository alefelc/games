  ...
  for (const endpoint of endpoints) {
    try {
      const result = await getJson<
        PublicBundleRecord | PublicBundleRecord[]
      >(endpoint, options.signal, { noStore: true });
...
const GAME_FIELDS = [
  'id','status','name','slug','tagline','description','minimum_age',
  'default_locale','active','theme','privacy_notice','stop_word',
  'terms_url','sort','cover_image',
];

const THEME_FIELDS = [
  'id','status','name','slug','primary_color','secondary_color',
  'accent_color','background_color','surface_color',
  'card_background_color','card_border_color','text_color',
  'muted_text_color','danger_color','heading_font_family',
  'body_font_family','card_font_family','heading_font_url',
  'body_font_url','card_font_url','border_radius','card_border_radius',
  'button_height','card_ratio','shadow_intensity','enable_card_flip',
  'enable_vibration','enable_sounds','enable_particles',
  'animation_speed','logo_file','favicon_file','app_icon_file',
];

const SETTINGS_FIELDS = [
  'id','game','status','default_mode','default_level',
  'start_screen_title','intro_text','instructions_text','safety_text',
  'stop_word','age_gate_enabled','show_timer',
  'allow_screen_wake_lock','allow_fullscreen','allow_vibration',
  'allow_offline','maximum_cards_per_session','enable_random_level',
  'enable_private_filters','analytics_enabled','maintenance_mode',
  'default_exclude_photo_video','default_exclude_third_parties',
  'default_exclude_public_places','default_exclude_restraint',
...
const LEVEL_FIELDS = [
  'id','game','status','name','slug','description','intensity_order',
  'color','icon','minimum_cards','recommended_duration_minutes',
  'requires_confirmation','sort','background_image',
];

const DECK_FIELDS = [
  'id','game','level','status','name','slug','description','deck_type',
  'minimum_players','maximum_players','active','sort','cover_image',
];

const MODE_FIELDS = [
  'id','game','status','name','slug','description','starting_level',
  'automatic_progression','cards_before_level_up',
  'allow_manual_level_change','turn_mode','skip_limit',
  'session_duration_minutes','repetition_policy','timer_policy','sort',
];

const ELEMENT_FIELDS = [
  'id','game','status','name','slug','category','description',
  'safety_instructions','is_consumable','is_optional','sort','image',
];

const TOY_FIELDS = [
  'id','game','status','name','slug','category','description',
  'intensity_min','difficulty','body_safe_notice','requires_cleaning',
  'cleaning_instructions','requires_lubricant','sort','image',
];

const TAG_FIELDS = [
  'id','game','status','name','slug','category','color','sort',
];
...
const CARD_FIELDS = [
  'id','game','level','status','sort','code','title','text',
  'instructions','card_type','original_deck','duration_seconds',
  'weight','intensity','minimum_players','maximum_players',
  'performer','target','performer_sex','target_sex',
  'anatomy_focus','anatomy_owner','penetration_method',
  'reciprocal_action','allow_skip',
  'requires_confirmation','safety_note','privacy_risk','physical_risk',
  'gender_scope','language','contains_oral','contains_penetration',
  'contains_anal','contains_restraint','contains_food',
  'contains_temperature','contains_public_place',
...
const DECK_CARD_FIELDS = [
  'id','deck','card','sort','enabled',
];

const CARD_ELEMENT_FIELDS = [
  'id','card','element','requirement','quantity',
  'preparation_note','sort',
];

const CARD_TOY_FIELDS = [
  'id','card','toy','requirement','quantity',
  'preparation_note','sort',
];

const CARD_TAG_FIELDS = [
  'id','card','tag','sort',
];
...
async function readFirst<T>(
  ...
  const result = await getJson<T[] | T>(
    endpoint,
    signal,
    { noStore: true },
  );
...
export async function readLiveCatalog(
  ...
  const cardIds = new Set(
    cards
      .map((row) =>
        relationId((row as { id?: unknown }).id),
      )
      .filter(Boolean),
  );
  ...
  return {
    ...
    cardElements: cardElements.filter((row) =>
      cardIds.has(
        relationId((row as { card?: unknown }).card),
      ),
    ),
