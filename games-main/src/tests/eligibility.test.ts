...
const emptyContent = {
  deckCards: [
    { id: "dc", deck: "deck-1", card: "card-1", sort: 1, enabled: true },
  ],
  cardElements: [],
  cardToys: [],
...
const context: EligibilityContext = {
  playerCount: 2,
  selectedLevelIds: new Set(["level-1"]),
  selectedDeckIds: new Set(["deck-1"]),
...
describe("eligibilidad de cartas", () => {
  it("acepta una carta compatible", () => {
    expect(
      isCardEligible(card, context, buildEligibilityIndexes(emptyContent)),
    ).toBe(true);
  });
  ...
  it("bloquea fotos cuando el filtro está activo", () => {
    expect(
      isCardEligible(
        { ...card, contains_photo: true },
        context,
        buildEligibilityIndexes(emptyContent),
      ),
    ).toBe(false);
  });
  ...
  it("exige elementos marcados como required", () => {
    ...
    expect(
      isCardEligible(
        card,
        { ...context, selectedElementIds: new Set(["hielo"]) },
        indexes,
      ),
    ).toBe(true);
  });
