...
describe("variedad persistente e intensidad v2.10.0", () => {
  it("recuerda cartas, prácticas y anatomías entre partidas", () => {
    const history = readFileSync("src/engine/card-history.ts", "utf8");
  ...
  it("sube varios niveles y conserva la reacción en el modo local", () => {
    ...
    const director = readFileSync("src/engine/game-master.ts", "utf8");

    expect(session).toContain("Math.min(4, 2 + recentRequests)");
    expect(director).toContain(`resolvedEvent?.reaction === "too_soft"`);
