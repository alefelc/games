...
function rankCandidates(
  ...
  const scored = candidates.map((card) => {
    ...
    value +=
      sessionRandom(
        `${session.id}:${session.resolvedCount}:rank:${card.id}`,
      ) * 10;
    return { card, value };
  });
  ...
  const groupList = [...groups.values()].sort((left, right) => {
    const leftId = left[0]?.card.id ?? "";
    const rightId = right[0]?.card.id ?? "";
    return (
      sessionRandom(
        `${session.id}:${session.resolvedCount}:group:${leftId}`,
      ) -
        sessionRandom(
          `${session.id}:${session.resolvedCount}:group:${rightId}`,
