...
/**
 * A manual level change is an explicit jump in the scene, not a preference
 * that may be silently ignored. The target level's own card contracts define
 * the scene preconditions, so this continues to work if levels are renamed or
 * their order changes.
 */
export function alignSceneForManualLevel(
  scene: SceneState,
  levelId: string,
  content: ContentBundle,
  setup?: GameSetup,
): SceneState {
  const allLevelCards = (content.cards ?? []).filter(
    (card) => card.status === "published" && card.level === levelId,
  );
  const filteredLevelCards = setup
    ? allLevelCards.filter((card) =>
        cardPassesDynamicFilters(
    ...
    : [];
  const levelCards = filteredLevelCards.length
    ? filteredLevelCards
    : allLevelCards;
  if (!levelCards.length) return scene;

  const entryCards = levelCards.filter(
    (card) =>
      !card.requires_previous_activity &&
      !["continue", "intensify"].includes(card.activity_action),
  );
  const representatives = entryCards.length ? entryCards : levelCards;
  const closureLevel =
    levelCards.filter((card) => card.card_kind === "closure").length >
    levelCards.length / 2;

  const phase = closureLevel
    ? "climax"
    : mostCommon(
        representatives.map((card) => card.scene_phase_min),
        scene.phase,
      );
  const physicalState = closureLevel
    ? "climax"
    : mostCommon(
        representatives.map((card) => card.physical_state_min),
        scene.physical_state,
      );
  const activeActivity = closureLevel
    ? mostCommon(
        representatives.map((card) => card.activity_family),
        scene.active_activity ?? "sensual-play",
      )
    : null;
