...
export function GameScreen({
  ...
  onCardPreference?: (cardId: Id, preference: Exclude<CardPreference, null>) => void;
}) {
  const card =
  ...
  const suggestedDuration = card?.duration_seconds ?? card?.estimated_duration_seconds ?? 0;
  const [timerRunning, setTimerRunning] = useState(false);
  const [remaining, setRemaining] = useState(suggestedDuration);
  const [cardPreference, setCurrentCardPreference] = useState<CardPreference>(
    card ? getCardPreference(card.id) : null,
  );
  ...
  useEffect(() => {
    ...
    setRemaining(card?.duration_seconds ?? card?.estimated_duration_seconds ?? 0);
    setTimerRunning(false);
    setCurrentCardPreference(card ? getCardPreference(card.id) : null);
  ...
  }, [card?.id, card?.duration_seconds, card?.estimated_duration_seconds]);
                ...
                {content.levels
                  ...
                  .map((item) => (
                    <button
                      key={item.id}
                      className={
                        (session.pendingLevelId ?? session.currentLevelId) ===
                        item.id
                          ? "active"
                          : ""
                      }
      ...
      <main className="game-main">
        ...
        {session.revealed ? (
          <>
            ...
            <div className="card-memory-actions" aria-label="Guardar preferencia de esta carta">
              {(
                [
                  ["favorite", "hearts", "Me gustó"],
                  ["later", "refresh", "Guardar"],
                  ["blocked", "close", "No repetir"],
                ] as const
              ).map(([preference, icon, label]) => (
                <button
                  key={preference}
                  type="button"
                  className={cardPreference === preference ? "selected" : ""}
                  aria-pressed={cardPreference === preference}
                  onClick={() => {
                    const next = cardPreference === preference ? null : preference;
                    setCardPreference(card.id, next);
                    setCurrentCardPreference(next);
                    if (next) onCardPreference?.(card.id, next);
                  }}
                >
                  <Icon name={icon} />
                  <span>{label}</span>
