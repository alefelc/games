...
export function SetupScreen({
  ...
  const eligibleCount = eligibleStats.sessionCapacity;
  const configuredMaximumCards = resolveMaximumCards(content.settings);
  const availableCardCapacity = Math.min(
    configuredMaximumCards,
    eligibleCount,
  );
  const maximumSelectableCards =
    availableCardCapacity >= 5
      ? Math.floor(availableCardCapacity / 5) * 5
      : availableCardCapacity;
  const cardRangeMinimum = Math.min(5, maximumSelectableCards || 5);
  const cardRangeMaximum = Math.max(
    cardRangeMinimum,
    maximumSelectableCards,
  );

  useEffect(() => {
    if (
      maximumSelectableCards <= 0 ||
      setup.maxCards <= maximumSelectableCards
    ) {
      return;
    }
    updateSetup({ maxCards: maximumSelectableCards });
  }, [maximumSelectableCards, setup.maxCards, updateSetup]);
        ...
        {step === 3 && setupPath === "custom" && (
          <section className="setup-section">
            ...
            <label className="range-row cards-range">
              ...
              <input
                type="range"
                min={cardRangeMinimum}
                max={cardRangeMaximum}
                step={cardRangeMaximum < 5 ? 1 : 5}
