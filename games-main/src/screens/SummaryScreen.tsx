...
export function SummaryScreen({
  ...
  return (
    <main className="center-screen summary-screen">
      ...
      <p>
        {isSolo
          ? "Era explorar a tu ritmo y disfrutar sin apuro."
          : "Era elegir juntos, respetar límites y disfrutar sin presión."}
      </p>

      {session && (
        <div className="summary-stats">
          <div>
            <b>{session.completedCardIds.length}</b>
            <span>cumplidas</span>
          </div>
          <div>
            <b>{session.skippedCardIds.length}</b>
            <span>saltadas</span>
          </div>
          <div>
            <b>{duration(session.startedAt, session.endedAt)}</b>
            <span>duración</span>
          </div>
        </div>
      )}
      ...
      <div className="summary-note">
        <Icon name="lock" />
        <span>
          Este resumen queda en este dispositivo y desaparece al iniciar otra
          partida.
        </span>
      </div>
      <div className="summary-actions">
        <button
          ...
          onClick={onRestart}
        >
          Configurar otra partida <Icon name="refresh" />
        </button>
      ...
      <small>
        {isSolo
          ? setup.playerOne || "Vos"
          : `${setup.playerOne || "Vos"} y ${setup.playerTwo || "tu pareja"}`}
      </small>
    </main>
  );
}
