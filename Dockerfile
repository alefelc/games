...
# El frontend vive en la raíz de este repositorio. Los paquetes compartidos
# se incluyen como workspaces locales para evitar depender del monorepo.
...
RUN npm run build \
 && printf '{"frontend_release":"%s","built_at":"%s"}\n' \
    "$BUILD_RELEASE" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > dist/build-info.json
